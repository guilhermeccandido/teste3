	...
	<div class="row">	  	  
		<div class="main">
          ...
          <div class="well">
            ...
            echo form_open("admin/relatorio_gerencial", $attributes);
	          ...
	          <table class="table table-striped table-bordered table-condensed">
	            ...
	            <tbody>
	              ...
	              foreach($relatorio_gerencial as $row)
	              {
					...
					if(file_exists(RELATORIOS_GERENCIAIS_AA4_FOLDER.'/'.$row['id'].'/pdf/'.$row['relatorio']) AND $row['relatorio'] != ''){
						$link .= '<a href="'.base_url().'assets/relatorios_gerenciais/'.$row['id'].'/pdf/'.$row['relatorio'].'" target="_blank">
								<img src="'.base_url().'assets/img/icons/relatorio.jpg"  />
							  </a> ';
					}
					 
	              	if(file_exists(RELATORIOS_GERENCIAIS_AA4_FOLDER.'/'.$row['id'].'/pdf/'.$row['portifolio']) AND $row['portifolio'] != ''){
						$link .= '<a href="'.base_url().'assets/relatorios_gerenciais/'.$row['id'].'/pdf/'.$row['portifolio'].'" target="_blank">
								<img src="'.base_url().'assets/img/icons/portifolio.jpg"  />
							  </a> ';
					}
					
					if(file_exists(RELATORIOS_GERENCIAIS_AA4_FOLDER.'/'.$row['id'].'/pdf/'.$row['carta'])  AND $row['carta'] != ''){
						$link .= '<a href="'.base_url().'assets/relatorios_gerenciais/'.$row['id'].'/pdf/'.$row['carta'].'" target="_blank">
								<img src="'.base_url().'assets/img/icons/carta.jpg"  />
							  </a> ';
					}
					
					if(file_exists(RELATORIOS_GERENCIAIS_AA4_FOLDER.'/'.$row['id'].'/colecao/'.$row['colecao'])  AND $row['colecao'] != ''){
						$link .= '<a href="'.base_url().'assets/relatorios_gerenciais/'.$row['id'].'/colecao/'.$row['colecao'].'" target="_blank">
								<img src="'.base_url().'assets/img/icons/rar.jpg"  />
							  </a> ';
					}
		          ...
		          echo '<a href="#myModal'.$row['id'].'" class="btn '.$btClass.'" data-toggle="modal" style="width: 130px;">Documentos</a>';
		          if($usuarioPerfil['acesso'] == 'editar'){ 	
					echo 
						'<a href="'.site_url("admin").'/relatorio_gerencial/update/'.$row['id'].'" class="btn btn-info" style="width: 130px;">Ver & editar</a>
