<?php
	...
	$options_responsaveis = array();
	foreach($responsaveis as $row)
	{
		$options_responsaveis[$row["id_usuario"]] = $row["nome"] ;
	}
...
<div class="container-fluid">		  
	<div class="row">	  	  
		<div class="main">
          ...
          <div class="row">
		<?php 
			foreach($pas_fases as $local => $userResp ) {
				//print_r($userResp);
		?>		
		   <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
	          <div class="panel panel-warning">
				<div class="panel-heading">
	              ...
	              <h3 class="panel-title"><?php echo $local; ?></h3>
	            </div>
	            <div class="panel-body" style="display:none;" >
	            <?php foreach($userResp as $user => $pas_fases ) {?>
					   <div class="table-responsive">
					   <h3><?php echo $options_responsaveis[$user]; ?></h3>
			            ...
			            foreach($pas_fases as $row)
			            {
			            	...
			            	echo "<tr class='".$row['classe']."' id='".$row['id']."' >";
					          ...
					          echo '<a href="'.site_url("admin").'/pas_fases_movimentacao/administrador/'.$row['id_pas'].'/'.$row['lastmov']['id_pas_fases'].'" class="btn btn-info" style="width: 130px;">Movimentar</a>';
					          /*
			    		  		  
				                  <a href="'.site_url("admin").'/pas_fases/update/'.$row['id'].'/'.$id_pas.'" class="btn btn-info" style="width: 130px;">Ver & editar</a>
				              */    
				                echo '</td>';
				                echo "</tr>";
	          ...
	          <table class="table table-striped table-bordered table-condensed  table-hover results">
	            ...
	            <tbody>
	              ...
	              foreach($pas as $row)
	              {
					...
					echo '	<button href=""  type="button" class="btn btn-info" aria-label="Visualizar" role="group" 
					...
					if($usuarioPerfil['acesso'] == 'editar'){ 					
						...
						echo   '<button href="#myModal"  type="button" class="btn btn-danger" aria-label="Deletar" data-toggle="modal" onclick="open_modal('.$row['id'].');" role="group"  >
...
google.load('visualization', '1.0', { packages:['timeline'], language:'pt-BR'});
	...
	$("#menu-toggle").click(function(e) {
 	...
 	/* AGENDA : ''*/
	var options = {
			events_source: "<?php echo ($id_contrato) ? base_url('pas/get_pas_planejamento_by_id_contrato').'/'.$id_contrato :  base_url('admin/pas/get_pas_planejamento_all_lotes'); ?>",
			view: 'year',
