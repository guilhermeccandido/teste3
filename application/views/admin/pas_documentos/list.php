	  ...
	  <div class="row">
        <div class="span12 columns">
          <div class="well">
            ...
            $options_pas_documentos = array();    
            foreach ($pas_documentos as $array) {
              foreach ($array as $key => $value) {
                $options_pas_documentos[$key] = $key;
              }
              break;
            }

            echo form_open("admin/pas_documentos", $attributes);
     
              echo form_label("Buscar:", "search_string");
              echo form_input("search_string", $search_string_selected);

              echo form_label("Ordenar por:", "order");
              echo form_dropdown("order", $options_pas_documentos, $order, 'class="span2"');
          ...
          <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
            	<th class="header">#</th>
				<th class="yellow header headerSortDown">PAS</th>
	            ...
	            <tbody>
	              <?php
	              foreach($pas_documentos as $row)
	              {
	                echo '<tr>';
	                echo '<td>'.$row['id'].'</td>';
					echo '<td>'.$row['id_pas'].'</td>';
	          ...
	          echo '<td class="crud-actions">
                  <a href="'.site_url("admin").'/pas_documentos/update/'.$row['id'].'" class="btn btn-info">Ver & editar</a>
	...
	<script>
		...
		function open_modal(id){
			$("#actionModal").attr("href", "pas_documentos/delete/"+id);
		}
