   ...
   <div class="container-fluid">		  
	<div class="row">	  	  
		<div class="main">
          ...
          <div class="well">
            ...
            $options_registro_financeiro = array();    
            foreach ($registro_financeiro as $array) {
              foreach ($array as $key => $value) {
                $options_registro_financeiro[$key] = $key;
              }
              break;
            }

            echo form_open("admin/registro_financeiro", $attributes);
              ...
              echo form_dropdown("order", $options_registro_financeiro, $order, 'class="form-control"');
          ...
          <div class="table-responsive">
	          <table class="table table-striped table-bordered table-condensed">
	            <thead>
	              <tr>
	            	<th class="header">#</th>
						<th class="yellow header headerSortDown">Financeiro</th>
						<th class="yellow header headerSortDown">Contrato</th>
						<th class="yellow header headerSortDown">Label Contrato</th>
						<th class="yellow header headerSortDown">Observações</th>
					</tr>
	            </thead>
	            <tbody>
	              <?php
	              foreach($registro_financeiro as $row)
	              {
	                echo '<tr>';
	                echo '<td>'.$row['id'].'</td>';
		          ...
		          echo '<td class="crud-actions">
	          			  <a href="'.site_url("admin").'/financeiro_medicoes/'.$row['id'].'" class="btn btn-info" style="width: 130px;">Medições</a>
		          		  <a href="'.site_url("admin").'/financeiro_fases_subfases/'.$row['id'].'" class="btn btn-info" style="width: 130px;">Produtos</a>
		          		  <a href="'.site_url("admin").'/financeiro_reajuste/'.$row['id'].'" class="btn btn-info" style="width: 130px;">Reajustes</a>
		                  <a href="'.site_url("admin").'/registro_financeiro/update/'.$row['id'].'" class="btn btn-info" style="width: 130px;">Ver & editar</a>
	...
	<script>
		...
		function open_modal(id){
			$("#actionModal").attr("href", "<?php echo site_url("admin")?>/registro_financeiro/delete/"+id);
		}

		/*
		function open_modal(id){
			$("#actionModal").attr("href", "registro_financeiro/delete/"+id);
		}		
		*/
	</script>