   ...
   <div class="container-fluid">		  
          ...
          <div class="table-responsive">
	          <table class="table table-striped table-bordered table-condensed">
	            <thead>
	              <tr>
	            	<th class="header">#</th>
					<th class="yellow header headerSortDown"><?php echo ucfirst("usuario_perfil"); ?></th><th class="yellow header headerSortDown">Status</th>
				<th class="yellow header headerSortDown">Descrição</th>
				</tr>
	        </thead>
	        <tbody>
              <?php
              foreach($status_perfil as $row)
              {
                echo "<tr>";
                echo "<td>".$row["id"]."</td>";
				echo "<td>".$row["usuario_perfil"]."</td>";
				echo '<td>'.$row['descricao'].'</td>';

		          echo '<td class="crud-actions">
	                  <a href="'.site_url("admin").'/status_perfil/update/'.$row['id'].'/'.$id_status.'" class="btn btn-info" style="width: 130px;">Ver & editar</a>
	...
	<script>
		...
		function open_modal(id, id2){
			$("#actionModal").attr("href", "<?php echo site_url("admin")?>/status_perfil/delete/"+id+"/"+id2);
		}
    
		/*
		function open_modal(id){
			$("#actionModal").attr("href", "status_perfil/delete/"+id);
		}
		*/
	</script>