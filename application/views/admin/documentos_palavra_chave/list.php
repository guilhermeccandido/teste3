   ...
   <div class="container-fluid">		  
	<div class="row">	  	  
		<div class="main">
          ...
          <div class="well">
            ...
            $options_documentos_palavra_chave = array();    
            foreach ($documentos_palavra_chave as $array) {
              foreach ($array as $key => $value) {
                $options_documentos_palavra_chave[$key] = $key;
              }
              break;
            }

            echo form_open("admin/documentos_palavra_chave", $attributes);
              ...
              echo form_dropdown("order", $options_documentos_palavra_chave, $order, 'class="form-control"');
          ...
          <div class="table-responsive">
	          <table class="table table-striped table-bordered table-condensed">
	            <thead>
	              <tr>
	            	<th class="header">#</th>
						<th class="yellow header headerSortDown">Palavra</th>
						<th class="yellow header headerSortDown">Observações</th>
	    			</tr>
	            </thead>
	            <tbody>
	              <?php
	              foreach($documentos_palavra_chave as $row)
	              {
	                echo '<tr>';
	                echo '<td>'.$row['id'].'</td>';
		          ...
		          echo '<td class="crud-actions">
	                  <a href="'.site_url("admin").'/documentos_palavra_chave/update/'.$row['id'].'" class="btn btn-info" style="width: 130px;">Ver & editar</a>
	...
	<script>
		...
		function open_modal(id){
			$("#actionModal").attr("href", "<?php echo site_url("admin")?>/documentos_palavra_chave/delete/"+id);
		}

		/*
		function open_modal(id){
			$("#actionModal").attr("href", "documentos_palavra_chave/delete/"+id);
		}		
		*/
	</script>