	  ...
	  <div class="row">
        <div class="span12 columns">
          <div class="well">
            ...
            $options_anteprojetos_documentos = array();    
            foreach ($anteprojetos_documentos as $array) {
              foreach ($array as $key => $value) {
                $options_anteprojetos_documentos[$key] = $key;
              }
              break;
            }

            echo form_open("admin/anteprojetos_documentos", $attributes);
     
              echo form_label("Buscar:", "search_string");
              echo form_input("search_string", $search_string_selected);

              echo form_label("Ordenar por:", "order");
              echo form_dropdown("order", $options_anteprojetos_documentos, $order, 'class="span2"');
          ...
          <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
	            ...
	            <tbody>
	              <?php
	              foreach($anteprojetos_documentos as $row)
	              {
	                echo '<tr>';
	                echo '<td>'.$row['id'].'</td>';
	          ...
	          echo '<td class="crud-actions">
                  <a href="'.site_url("admin").'/anteprojetos_documentos/update/'.$row['id'].'" class="btn btn-info">Ver & editar</a>
	...
	<script>
		...
		function open_modal(id){
			$("#actionModal").attr("href", "anteprojetos_documentos/delete/"+id);
		}
