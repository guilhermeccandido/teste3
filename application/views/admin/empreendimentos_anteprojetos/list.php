	  ...
	  <div class="row">
        <div class="span12 columns">
          <div class="well">
            ...
            $options_empreendimentos_anteprojetos = array();    
            foreach ($empreendimentos_anteprojetos as $array) {
              foreach ($array as $key => $value) {
                $options_empreendimentos_anteprojetos[$key] = $key;
              }
              break;
            }

            echo form_open("admin/empreendimentos_anteprojetos", $attributes);
     
              echo form_label("Buscar:", "search_string");
              echo form_input("search_string", $search_string_selected);

              echo form_label("Ordenar por:", "order");
              echo form_dropdown("order", $options_empreendimentos_anteprojetos, $order, 'class="span2"');
          ...
          <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
				...
				<th class="yellow header headerSortDown">Anteprojeto</th>
	            ...
	            <tbody>
	              <?php
	              foreach($empreendimentos_anteprojetos as $row)
	              {
	                echo '<tr>';
	                echo '<td>'.$row['id'].'</td>';
					echo '<td>'.$row['codigo'].'</td>';
					echo '<td>'.$row['id_empreendimento'].'</td>';
					echo '<td>'.$row['id_anteprojeto'].'</td>';
	          ...
	          echo '<td class="crud-actions">
                  <a href="'.site_url("admin").'/empreendimentos_anteprojetos/update/'.$row['id'].'" class="btn btn-info" style="width: 130px;">Ver & editar</a>
	...
	<script>
		...
		function open_modal(id){
			$("#actionModal").attr("href", "empreendimentos_anteprojetos/delete/"+id);
		}
