   ...
   <div class="container-fluid">		  
	<div class="row">	  	  
		<div class="main">
          ...
          <div class="well">
            ...
            $options_documentos_tipos = array();    
            foreach ($documentos_tipos as $array) {
              foreach ($array as $key => $value) {
                $options_documentos_tipos[$key] = $key;
              }
              break;
            }

            echo form_open("admin/documentos_tipos", $attributes);
              ...
              echo form_dropdown("order", $options_documentos_tipos, $order, 'class="form-control"');
          ...
          <div class="table-responsive">
	          <table class="table table-striped table-bordered table-condensed">
	            ...
	            <tbody>
	              <?php
	              foreach($documentos_tipos as $row)
	              {
	                echo '<tr>';
	                echo '<td>'.$row['id'].'</td>';
					echo '<td>'.$row['titulo'].'</td>';
					echo '<td>'.$options_modulos[$row['modulo']].'</td>';
					echo '<td>'.$row['observacoes'].'</td>';
					
		          echo '<td class="crud-actions">
	                  <a href="'.site_url("admin").'/documentos_tipos/update/'.$row['id'].'" class="btn btn-info" style="width: 130px;">Ver & editar</a>
	...
	<script>
		...
		function open_modal(id){
			$("#actionModal").attr("href", "<?php echo site_url("admin")?>/documentos_tipos/delete/"+id);
		}

		/*
		function open_modal(id){
			$("#actionModal").attr("href", "documentos_tipos/delete/"+id);
		}		
		*/
	</script>