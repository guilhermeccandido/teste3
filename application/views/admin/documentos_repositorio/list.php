   ...
   <div class="container-fluid">		  
		...
		<div class="main">
          ...
          <div class="well">
            ...
            foreach($tipo_documento as $item){
            	$options_tipos[$item['id']] = $item['titulo'];
            }
            ...
            $options_documentos_repositorio = array();    
            foreach ($documentos_repositorio as $array) {
              foreach ($array as $key => $value) {
                $options_documentos_repositorio[$key] = $key;
              }
              break;
            }

            echo form_open("admin/documentos_repositorio", $attributes);
              ...
              echo form_dropdown("order", $options_documentos_repositorio, $order, 'class="form-control"');
          ...
          <div class="table-responsive">
	          <table class="table table-striped table-bordered table-condensed">
	            <thead>
	              <tr>
	            	<th class="header">#</th>
				<th class="yellow header headerSortDown">Assunto</th>
				<th class="yellow header headerSortDown">Tipo do Documento</th>
				<th class="yellow header headerSortDown">Número do Documento</th>
				<th class="yellow header headerSortDown">Data do Documento</th>
				...
				<th class="yellow header headerSortDown">Lista de Palavras Chave</th>
	            ...
	            <tbody>
	              <?php
	              foreach($documentos_repositorio as $row)
	              {
	                echo '<tr>';
	                echo '<td>'.$row['id'].'</td>';
					echo '<td>'.$row['titulo'].'</td>';
					echo '<td>'.$options_tipos[$row['tipo_documento']].'</td>';
					echo '<td>'.$row['n_documento'].'</td>';
					echo '<td>'.$row['data'].'</td>';
					echo '<td>'.$row['uf'].'</td>';
					echo '<td>'.$row['rodovia'].'</td>';
					echo '<td>'.$options_atividades[$row['atividade']].'</td>';
					echo '<td>'.$row['palavra_chave'].'</td>';
					echo '<td>'.$row['origem'].'</td>';
					echo '<td>'.$row['destino'].'</td>';
					echo '<td>'.$row['observacoes'].'</td>';
					
		          echo '<td class="crud-actions">
	                  <a href="'.site_url("admin").'/documentos_repositorio/update/'.$row['id'].'" class="btn btn-info" style="width: 130px;">Ver & editar</a>
	...
	<script>
		...
		function open_modal(id){
			$("#actionModal").attr("href", "<?php echo site_url("admin")?>/documentos_repositorio/delete/"+id);
		}

		/*
		function open_modal(id){
			$("#actionModal").attr("href", "documentos_repositorio/delete/"+id);
		}		
		*/
	</script>