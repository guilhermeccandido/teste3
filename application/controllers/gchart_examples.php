...
class Gchart_examples extends CI_Controller
{
    ...
    public function timeline_chart_basic()
    {
    	$this->gcharts->load('TimelineChart');
    
    	$this->gcharts->DataTable('Stocks')
	    	->addColumn('string', 'Count', 'count')
	    	->addColumn('string', 'Name', 'name')
	    	->addColumn('date', 'Start', 'start')
	    	->addColumn('date', 'End', 'end');
    	...
    	for($a = 1; $a < 25; $a++)
    	{
    		...
    		$this->gcharts->TimelineChart('Stocks')->setConfig($config);
    
    		$this->load->view('gcharts/timeline_chart_basic');
    }
